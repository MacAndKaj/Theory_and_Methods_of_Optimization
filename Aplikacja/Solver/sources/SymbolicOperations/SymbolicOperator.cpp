    ...
    const std::shared_ptr<FunctionWrapper>& wrapper) const
{
    if (scriptname.empty())
    {
        _log << "E["+std::string(__FUNCTION__) + "]| Creating python file failed!";
        return {};
    }
    ...
    if (file.bad())
    {
        _log << "E["+std::string(__FUNCTION__) + "]| Reading from file derivatives went wrong!";
        system(command.c_str());
        return {};
    }
    GradientVector ret;
    ...
    {
        _log << "I["+std::string(__FUNCTION__) + "] derivative " + buffer;
        ret.emplace_back(
            _functionsFactory->getFunctionFromString(buffer, wrapper->getDimension()));
    }
    file.close();
    system(command.c_str());
    _log << "I["+std::string(__FUNCTION__) + "]| Completed successfully";
    return ret;
}
